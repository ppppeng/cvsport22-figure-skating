...
from out_score import out_file
...
def train_model(cfg,
    ...
    valid_batch_size = cfg.DATASET.get('val_batch_size', batch_size)
    ...
    mkdir(output_dir)
    valid_log_file = osp.join(output_dir, 'valid.csv')  # todo
    ...
    train_loader = build_dataloader(train_dataset, **train_dataloader_setting)  # todo
    if validate:
    ...
    for epoch in range(0, cfg.epochs):
        ...
        # train_dataset.pipeline.pipelines[0].refresh_window_size()
        ...
        def evaluate(best):
            ...
            for top_flag in ['hit_at_one', 'top1']:
                if record_list.get(top_flag):  # todo my record
                    valid_log.append((epoch, record_list[top_flag].avg))
                if record_list.get(
    ...
    valid_log.append(('best', best))
    out_file(valid_log, valid_log_file)
