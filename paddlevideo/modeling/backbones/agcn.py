...
class GCN(nn.Layer):
    def __init__(self, in_channels, out_channels, vertex_nums=25, stride=1):
        ...
        self.conv1 = nn.Conv2D(in_channels=in_channels,
                               out_channels=out_channels * 3,  # TODO debug code
                               kernel_size=1,
                               stride=1)
        self.conv2 = nn.Conv2D(in_channels=vertex_nums * 3,  # TODO debug code
    ...
    def forward(self, x):
        ...
        # x = self.conv1(x)  # N,C,T,V
        # x = paddle.transpose(x, perm=[0, 3, 1, 2])  # N,V,C,T
        # x = self.conv2(x)
        # x = paddle.transpose(x, perm=[0, 2, 3, 1])  # N,C,T,V
        # return x
...
class AGCN(nn.Layer):
    ...
    def __init__(self, in_channels=2, **kwargs):
        ...
        # self.data_bn = nn.BatchNorm1D(25 * 2)   # unused code
        self.agcn = nn.Sequential(
            ...
            Block(in_channels=64, out_channels=64, **kwargs),
            Block(in_channels=64, out_channels=64, **kwargs),
            Block(in_channels=64, out_channels=128, stride=2, **kwargs),  # TODO debug code
            Block(in_channels=128, out_channels=128, **kwargs),
            Block(in_channels=128, out_channels=128, **kwargs),
            Block(in_channels=128, out_channels=256, stride=2, **kwargs),  # TODO debug code
            Block(in_channels=256, out_channels=256, **kwargs),
            Block(in_channels=256, out_channels=256, **kwargs),

            Block(in_channels=256, out_channels=512, stride=2, **kwargs),  # TODO debug code
            Block(in_channels=512, out_channels=512, **kwargs),
            Block(in_channels=512, out_channels=512, **kwargs)
        )
